...
void GazeboRosRealsense::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  this->dr_server = new dynamic_reconfigure::Server<realsense_gazebo_plugin::RealsenseGazeboPluginConfig>(ros::NodeHandle(*this->rosnode_, "config"));
  dynamic_reconfigure::Server<realsense_gazebo_plugin::RealsenseGazeboPluginConfig>::CallbackType cb;
  cb = boost::bind(&GazeboRosRealsense::dynamic_reconfigure_cb, this, _1, _2);
  this->dr_server->setCallback(cb);
}


void GazeboRosRealsense::dynamic_reconfigure_cb(realsense_gazebo_plugin::RealsenseGazeboPluginConfig &config, uint32_t level)
{
  if (level == 0){
    RealSensePlugin::SetCameraNoise(config.noise);

    // 0 hz means run as fast as possible, but we want no publishing
    RealSensePlugin::SetCameraUpdateRate(config.frame_rate == 0 ? 0.001 : config.frame_rate);
  }
}
